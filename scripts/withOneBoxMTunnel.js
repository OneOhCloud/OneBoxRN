...
// Ruby 脚本内容内嵌，避免在 EAS 构建时因文件路径变化导致脚本丢失
const RUBY_SCRIPT = `#!/usr/bin/env ruby
...
# 路径配置 - SDK 55+ 目录结构适配
# ARGV[1] 为原始模块 iOS 源码目录（由 JS 插件传入，兼容 EAS 临时构建路径）
MODULE_IOS_DIR = ARGV[1] || File.expand_path(__dir__)   # src/modules/expo-onebox/ios
PROJECT_ROOT = File.expand_path('../../../../', MODULE_IOS_DIR)  # 项目根目录
...
EXTENSION_SOURCE = File.expand_path('OneBoxMTunnel', MODULE_IOS_DIR)
LIBBOX_FRAMEWORK = File.expand_path('Libbox.xcframework', MODULE_IOS_DIR)
...
const withOneBoxMTunnel = (config) => {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      const scriptPath = path.join(tmpDir, `add_onebox_tunnel_${Date.now()}.rb`);

      console.log('[withOneBoxMTunnel] 正在配置 Network Extension...');
      console.log(`[withOneBoxMTunnel] iOS 目录: ${iosDir}`);
      console.log(`[withOneBoxMTunnel] 临时脚本路径: ${scriptPath}`);
